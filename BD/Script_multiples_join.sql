...
select * from ((((((((((tb_reportedisciplinar inner join tb_alumnos on tb_reportedisciplinar.r_alumno = tb_alumnos.idTb_Alumnos) join tb_personal on tb_reportedisciplinar.r_personal = tb_personal.idTb_Personal)
 join tb_materia on tb_reportedisciplinar.r_materia = tb_materia.idTb_Materia)inner join ct_incidente on tb_reportedisciplinar.r_tipoincidente = ct_incidente.idCt_incidente)
...
inner join tb_personal as tb2 on tb_reportedisciplinar.r_personalsolicita = tb2.idTb_Personal);
 select count (idTb_ReporteDisciplinar) as suma
 from  tb_reportedisciplinar 
 where r_alumno = 2;
 
 select * from tb_reportedisciplinar;
 
 select count(idTb_ReporteDisciplinar) as suma from tb_reportedisciplinar where r_alumno = 2;
