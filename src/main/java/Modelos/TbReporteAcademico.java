/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package Modelos;

/**
 *
 * @author Jonat
 */
public class TbReporteAcademico {
    private int idtbreporte;
    private int rpersonal;
    private int rmateria;
    private int rsemana;
    private int ralumnohonor;
    private int ralumnoatencion;
    private int ratencion;
    private int status;
    private int tipoescuela;

    public int getIdtbreporte() {
        return idtbreporte;
    }

    public void setIdtbreporte(int idtbreporte) {
        this.idtbreporte = idtbreporte;
    }

    public int getRpersonal() {
        return rpersonal;
    }

    public void setRpersonal(int rpersonal) {
        this.rpersonal = rpersonal;
    }

    public int getRmateria() {
        return rmateria;
    }

    public void setRmateria(int rmateria) {
        this.rmateria = rmateria;
    }

    public int getRsemana() {
        return rsemana;
    }

    public void setRsemana(int rsemana) {
        this.rsemana = rsemana;
    }

    public int getRalumnohonor() {
        return ralumnohonor;
    }

    public void setRalumnohonor(int ralumnohonor) {
        this.ralumnohonor = ralumnohonor;
    }

    public int getRalumnoatencion() {
        return ralumnoatencion;
    }

    public void setRalumnoatencion(int ralumnoatencion) {
        this.ralumnoatencion = ralumnoatencion;
    }

    public int getRatencion() {
        return ratencion;
    }

    public void setRatencion(int ratencion) {
        this.ratencion = ratencion;
    }

    public int getStatus() {
        return status;
    }

    public void setStatus(int status) {
        this.status = status;
    }

    public int getTipoescuela() {
        return tipoescuela;
    }

    public void setTipoescuela(int tipoescuela) {
        this.tipoescuela = tipoescuela;
    }
    
}
